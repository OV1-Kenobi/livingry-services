...
export const CATEGORIES: CategoryAnatomy[] = [
  ...
  },
  {
    ...
    y: 16,
    purpose:
      "Turn verified delivery into approved proof, attributed referrals, and the next appropriate customer opportunity while routing unresolved concerns to a human first.",
    inputs: ["Verified completion", "Customer contact preferences", "Complaint, callback & billing status", "Approved evidence"],
    processing: ["Check eligibility", "Route service-recovery exceptions", "Prepare human-approved requests", "Capture referral context", "Package approved proof"],
    outputs: ["Approved review & proof record", "Attributed referred lead", "Repeat-service opportunity", "Human-owned service-recovery case"],
    humanIntervention: ["Approve customer outreach", "Own negative-feedback cases", "Approve public proof"],
    successMeasures: ["Eligible completions reaching feedback", "Approved proof captured", "Attributed referrals routed", "Service-recovery cases resolved", "New demand influenced by proof"],
    capabilities: ["Completion eligibility", "Feedback and recovery routing", "Review requests", "Referral capture", "Proof approval ledger", "Discovery handoff"],
    workflowParticipation: ["completed-job-to-proof"],
    permissions: ["No fabricated proof or rating manipulation", "No public use without approval", "No outreach when an unresolved exception blocks it"],
    approvalRequirements: ["Human approval before customer outreach", "Human clearance after service recovery", "Human approval before proof publication"],
    integrationKeys: ["marketing_crm", "content_creation"],
  },
  {
    id: "recovery",
...
export const FLOW_EDGES: FlowEdge[] = [
  ...
  { from: "tradeops", to: "review_referral", label: "verified completion" },
  { from: "review_referral", to: "discovery_trust", label: "approved proof" },
  { from: "review_referral", to: "response", label: "referred lead" },
  { from: "review_referral", to: "continuity", label: "repeat-service opportunity" },
  ...
  { from: "review_referral", to: "ops_layer", label: "events" },
...
export const CANDIDATE_CATALOG: Record<CategoryId, CandidateTool[]> = {
  ...
  review_referral: [
    { id: "generic-review-request", name: "Review request workflow", neutral: true, capability: "Send timely, neutral review requests to appropriate destinations." },
    { id: "generic-referral-capture", name: "Referral capture & attribution", neutral: true, capability: "Capture referral context, permission, and attribution." },
    { id: "generic-proof-ledger", name: "Proof approval ledger", neutral: true, capability: "Store approved customer language and evidence with usage permissions." },
  ],
  recovery: [
