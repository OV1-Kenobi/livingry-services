...
// The comparison is constant-time (timingSafeEqual) with a length guard so a
// wrong-length secret refuses without leaking timing information
// (SEC-2026-009). The secret is read from env — rotation via env, unchanged.
export async function POST(req: NextRequest) {
	const secret = req.headers.get("x-setup-secret");
	if (!process.env.SETUP_SECRET || !secret) {
		return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
	}
	const expected = Buffer.from(process.env.SETUP_SECRET, "utf8");
	...
	const existing = await query("select id from tenants where slug = $1", [
		tenantSlug,
	]);
	...
	const seedRows: Array<
		[string, string, string, number, number | null, string | null, string]
	> = [
		[
			"booking_scheduling",
			"victoria_ai",
		...
		[
			...
			30,
			null,
			"Content creation to attract/build trust. $290/mo for 10 seats billed annually, 30-day free trial. Delay until first paying client.",
		],
		[
		...
		[
			...
			"Email/SMS/WhatsApp marketing + CRM. $8.08/mo. Core to intake gateway confirmations — activate first alongside Victoria AI.",
		],
		[
			"sales_outreach",
			"aura_app_ai",
			...
			14,
			"https://app.aura-app.ai",
			"AI Sales Performance Platform. $97/mo + 20% commission, 2-week free trial. Delay until pipeline exists.",
		],
		[
			"orchestration",
			"n8n",
			...
			"AI agent/workflow orchestration core for the entire TradeOps layer. $24-60/mo, 2-week free trial. Activate as soon as dogfood workflows are ready to wire.",
		],
	];
