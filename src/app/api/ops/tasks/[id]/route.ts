...
export async function PATCH(
	...
	if (b.snooze_reason !== undefined) push("snooze_reason", b.snooze_reason);
	if (b.status === "completed") push("completed_at", new Date());
	...
	const { rows } = await query(
		`update tasks set ${sets.join(", ")} where id = $${idx} and tenant_id = $${idx + 1} returning *`,
		values,
	);
	if (rows.length === 0)
