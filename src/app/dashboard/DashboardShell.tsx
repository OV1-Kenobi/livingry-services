...
// Client shell for the private authenticated dashboard. The wrapping
// server layout (layout.tsx) owns the noindex metadata and renders this
// shell, so every /dashboard/* route inherits the crawler-exclusion
// contract without each client page needing its own metadata export.
      ...
      <section style={{ borderBottom: "1px solid var(--rule)", background: "var(--paper-2)" }}>
        <div className="container" style={{ paddingBlock: "0.9rem" }}>
          ...
          <nav className="mt-3 flex flex-wrap gap-2" aria-label="Dashboard sections">
            ...
            {/* Decision 28b: Habitats-OS coordination link lives in this PRIVATE
                authenticated dashboard only — never in public nav/footer/sitemap.
                Founder-supplied sister-site domain 2026-08-23 (not yet active):
                ops.habitats-os.com */}
            <a href="https://ops.habitats-os.com" className="btn btn-secondary" style={{ paddingBlock: "0.45rem", paddingInline: "0.9rem", fontSize: "0.82rem" }}>Habitats-OS</a>
          </nav>
        </div>
      </section>
