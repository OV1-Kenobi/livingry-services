...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem("livingry_tenant_id");
    if (stored) {
      setTenantId(stored);
    } else {
      setLoading(false);
  ...
  async function load() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/ops/integrations?tenant_id=${tenantId}`);
  ...
  async function runSetup() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(await res.text());
      const data = await res.json();
      window.localStorage.setItem("livingry_tenant_id", data.tenant_id);
